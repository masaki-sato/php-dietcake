...
class CurlRequest {
    ...
    public function buildCurlOptions($method, $url, $params = null, $headers = null) {
        ...
        if ($method == 'GET') {
            ...
            elseif (is_file($params)) $options[CURLOPT_POSTFIELDS] = file_get_contents($params); elseif ($params) $options[CURLOPT_POSTFIELDS] = $params;
            // method の 決定
            if ($method == 'POST') $options[CURLOPT_POST] = 1;
            elseif (in_array($method, array('DELETE', 'PUT'))) $options[CURLOPT_CUSTOMREQUEST] = $method; else                                                         throw new Exception ('Invalid Method:' . $method);
        }
